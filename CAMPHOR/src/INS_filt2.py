...
min_prop_distance_from_edge = float(sys.argv[4])
min_segment_length = int(sys.argv[5])
mapper_filter = int(sys.argv[6])

ins_col_num = int(sys.argv[7])

min_read_num = int(sys.argv[8])
min_read_with_segment_num = int(sys.argv[9])
min_minimap2_read_num = int(sys.argv[10])
...
#		print("read_tmp_l => ", read_tmp_l)
		...
		if re.search("split", read_tmp_l[1]):
...
#			print("mapper_l", mapper_l)
			if mq_l[0] != "NA" and int(mq_l[0]) < min_mq:
				continue
			elif mq_l[1] != "NA" and int(mq_l[1]) < min_mq:
				continue

			if read_tmp_l[3] != "NA" and abs(int(line_l[1]) - int(read_tmp_l[3])) > min_distance_cutoff:
				continue
			elif read_tmp_l[5] != "NA" and abs(int(line_l[3]) - int(read_tmp_l[5])) > min_distance_cutoff:
				continue

			pass_reads_num += 1

			if (start_l[0] != "NA" and start_l[1] != "NA") and abs(int(end_l[0]) - int(start_l[0])) >= min_segment_length and abs(int(end_l[1]) - int(start_l[1])) >= min_segment_length:
				num_long_reads += 1
			elif start_l[1] == "NA" and abs(int(end_l[0]) - int(start_l[0])) >= min_segment_length:
#				print("min_segment_length", start_l, end_l)
#				print(int(end_l[0]) - int(start_l[0]), int(end_l[1]) - int(start_l[1]))
				num_long_reads += 1
			elif start_l[0] == "NA" and abs(int(end_l[1]) - int(start_l[1])) >= min_segment_length:
				num_long_reads += 1

			if mapper_l[0] == "-" and mapper_l[1] == "-":
#				print("mapper_l", mapper_l)
				minimap2_reads += 1
			elif mapper_l[1] == "NA" and mapper_l[0] == "-":
				minimap2_reads += 1
			elif mapper_l[0] == "NA" and mapper_l[1] == "-":
				minimap2_reads += 1
		
		if read_tmp_l[1] == "within":
#			print("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
#			print("mq", read_tmp_l[12])
#			print("dis", read_tmp_l[3])
...
#				print("min_distance", line_l[1], line_l[3], read_tmp_l[3], read_tmp_l[5], int(line_l[1]) - int(read_tmp_l[3]), int(line_l[3]) - int(read_tmp_l[5]))
				continue
...
#		print("PASS!!")
#	else:
#		print("DROP !!", pass_reads_num, num_long_reads, minimap2_reads)
#		print(">>>>>", line_l[8])
